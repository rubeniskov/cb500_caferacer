...
{
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, 0); //dc
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, 0); //cs
	_sendData(c);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, 1); //cs
}

void _sendData(uint8_t c) {
	//SPI_I2S_SendData(SPI1, c);
    //HAL_SPI_TransmitReceive_IT(spiHandle, )
    if (HAL_SPI_Transmit_IT(&hspi1, (uint8_t *)&c, sizeof(c)) != HAL_OK) {
        Error_Handler();
    };
...
void SSD1331_drawPixel(uint16_t x, uint16_t y, uint16_t color)
{
	...
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, 1); //dc
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, 0); //cs
